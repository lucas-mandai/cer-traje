            ...
            {features.map(({ id, icon, caption, title, text, button }) => (
              ...
              >
                <div className="w-full flex justify-start items-start">
                  <div className="-ml-3 mb-3 flex items-center justify-center flex-col">
                ...
                <p className="caption mb-5 mt-7 max-md:mb-6">{caption}</p>
                <h2 className="max-w-400 mb-7 h4 text-p4 max-md:mb-6 max-md:h5">
                  {title}
                </h2>
                <p className="mb-11 body-1 max-md:mb-8 max-md:body-3">{text}</p>
                <LinkScroll to="video" offset={-100} spy smooth>
                  <Button icon={button.icon}>{button.title}</Button>
                </LinkScroll>
              </div>
            ))}

            <ul className="relative flex justify-around flex-grow flex-wrap px-[5%] border-2 border-s3 rounded-7xl ">
              <div className="absolute bg-s3/20 top-[38%] left-0 right-0 w-full h-[1px] z-10" />

              {details.map(({ id, icon, title }) => (
                <li key={id} className="relative pt-6 pb-6 md:pt-16 px-0 md:pb-14 ">
